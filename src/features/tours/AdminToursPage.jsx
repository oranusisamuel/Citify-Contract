...
import { deleteTourRequest, fetchToursPage, updateTourStatus } from './toursStore'

const statusOptions = ['new', 'contacted', 'scheduled', 'closed']
const TOURS_BATCH_SIZE = 50
...
const AdminToursPage = () => {
  ...
  const [toursCursor, setToursCursor] = useState(null)
  ...
  useEffect(() => {
    const loadInitialTours = async () => {
      ...
      try {
        const { tours: firstPage, cursor, hasMore } = await fetchToursPage({
          pageSize: TOURS_BATCH_SIZE,
        })
        setTours(firstPage)
        setToursCursor(cursor)
        setHasMoreTours(hasMore)
      } catch {
  ...
  const loadMoreTours = async () => {
    if (isLoadingMore || !hasMoreTours || !toursCursor) return

    setIsLoadingMore(true)
    try {
      const { tours: nextPage, cursor, hasMore } = await fetchToursPage({
        pageSize: TOURS_BATCH_SIZE,
        cursor: toursCursor,
      })
      ...
      setError('Unable to load more inspection requests.')
    } finally {
  ...
  useEffect(() => {
    ...
    const latest = tours.find((tour) => tour.id === selectedTour.id)
    if (!latest) {
          ...
          {!loading && tours.length > 0 && (
            <div className='space-y-4'>
              <div className='rounded-2xl border border-slate-200 bg-white p-3 md:p-4 space-y-3'>
                ...
                <div className='flex flex-wrap gap-2 overflow-x-auto md:grid md:grid-cols-5 md:overflow-visible pb-1 md:pb-0'>
                  {[
                    { value: 'all', label: 'All' },
                    { value: 'new', label: 'New' },
                ...
                <p className='text-xs text-slate-500'>Showing {filteredTours.length} loaded request{filteredTours.length !== 1 ? 's' : ''}</p>
              </div>
              ...
              <div className='rounded-2xl border border-slate-200 bg-white overflow-hidden'>
                ...
                {hasMoreTours && (
                  <div className='flex items-center justify-center px-3 md:px-4 py-3 border-t border-slate-100 bg-white'>
                    <button
                      type='button'
                    ...
                    >
                      {isLoadingMore ? 'Loading more...' : 'Load More Inspections'}
                    </button>
                  </div>
                )}
              </div>
